...
- Action settlement policy: in real multiplayer local auto-mode, requested combat actions are treated as issued immediately and the controller waits for the synchronized queue instead of the local request object's `CompletionTask`. This avoids requiring F4 toggles to clear stale `PlayCardAction:None` request state.
