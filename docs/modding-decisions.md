...
- Reward UI policy: while host auto-mode is enabled, the host also resolves room-end rewards deterministically, but the host reward offer is allowed to open through the normal foreground UI before auto-selection begins. Gold/relic/potion rewards are collected automatically, and card rewards use the same build-aware evaluator as AI teammates, including skip decisions for weak or off-build offers.
...
- Action settlement policy: in real multiplayer local auto-mode, requested combat actions are treated as issued immediately and the controller waits for the synchronized queue instead of the local request object's `CompletionTask`. A short synchronized-queue grace is applied before replanning so the host echo can update hand/energy before the next local request. This avoids requiring F4 toggles to clear stale `PlayCardAction:None` request state without flooding the host with back-to-back actions.
...
- Serialization policy: run telemetry sanitizes non-finite floating-point values before writing JSON. Some planners use sentinel values such as negative infinity internally, but the stored run file must remain valid and should not be lost because one score is non-finite.
...
## Remove Multiplayer Player Limit Support
...
- UI policy: foreground Auto-Mode reward collection now releases the visible `NRewardButton` instead of selecting the `Reward` model directly. This keeps the loot window, reward animations, and top-bar counters on the game's normal synchronized path.
- Card choice policy: foreground card rewards temporarily allow the original card reward screen to open, then feed the build-scored card index back after a short delay so the player can see the card choice before it is taken or skipped.
...
- Follow-up tuning: the line planner now boosts setup/draw actions when choosing initial beam candidates, because otherwise low-immediate-value draw/setup cards could be discarded before terminal line scoring saw them. Lines that leave setup unplayed after spending a weak starter Strike are penalized.
- Potion policy: generic or unclassified potions receive emergency value at low HP or high uncovered damage, so `death_with_unused_potions` should decrease even when a potion id is not recognized as purely offensive or defensive.
